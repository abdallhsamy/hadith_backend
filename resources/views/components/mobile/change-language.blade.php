...
{{--    <button class="text-primary bg-white px-2 py-1 rounded-lg border border-primary hover:bg-shade">--}}
...
{{--            <a class="py-2 px-4 hover:bg-shade hover:font-semibold" href="#" data-language="$language->code">{{ $language->native }}</a>--}}

{{--        @endforeach--}}
{{--    </div>--}}
    </div>
                <select id="change_system_language" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1 h-8">
                    <option selected>
                                {{ __('general.change_language') }}
                    </option>
                    @foreach($availableLanguages as $language)
                        <option value="{{ route('change-language', $language->code) }}">{{ $language->native }}</option>
                    @endforeach
                </select>

    @push('scripts')
        <script>
            document.querySelector('#change_system_language').addEventListener('change', function() {
